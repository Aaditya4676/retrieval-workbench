...
I encountered a concrete runtime issue: the full length-constrained decoder schema stopped the local runner, while a structural decoder schema succeeded with the same context. I retained all length and citation validation in the application and preserved the failed probe. A single local generation evaluation is being recorded independently; tomorrow's hosted answer model requires a new generation evaluation while the permanent MiniLM retrieval reference remains unchanged.
